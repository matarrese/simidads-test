/**
 * Contains logic for sending messages between the SIMID creative and the player.
...
class SimidProtocol {
  constructor() {
    /**
     * A map of message type to an array of callbacks.
  ...
  sendMessage(messageType, messageArgs) {
    ...
    const nameSpacedMessage =
      messageType == ProtocolMessage.CREATE_SESSION
        ? messageType
        : SimidPrefix.SIMID_PREFIX + messageType;
  ...
  receiveMessage(event) {
    ...
    } else if (type.startsWith(SimidPrefix.SIMID_PREFIX)) {
      // Remove SIMID: from the front of the message so we can compare them with the map.
      const specificType = type.substr(SimidPrefix.SIMID_PREFIX.length);
   ...
   */
  createSession() {
    const sessionCreationResolved = function () {
      // Session was created
    };
    const sessionCreationRejected = function () {
   ...
   * @param {!Element} target the new target of the message.
   */
